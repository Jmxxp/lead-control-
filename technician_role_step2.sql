...
begin
  ...
  if v_session.user_role not in ('admin', 'technician') then
    raise exception 'Apenas admin ou tecnico pode alterar opcoes.';
  end if;
...
begin
  ...
  if v_session.user_role not in ('admin', 'technician') then
    raise exception 'Apenas admin ou tecnico pode alterar categorias.';
  end if;
